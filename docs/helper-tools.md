# Вспомогательные инструменты технического писателя

## Работа с API в Postman и curl

- [Что такое Postman и как его использовать в тестировании API](https://ru.hexlet.io/blog/posts/postman)
- [Postman: что это такое и как им пользоваться](https://skillbox.ru/media/code/postman-chto-eto-takoe-i-kak-im-polzovatsya/)
- [Postman Docs](https://learning.postman.com/docs/getting-started/overview/)
- [Understand curl more](https://idratherbewriting.com/learnapidoc/docapis_understand_curl.html)
- [Глубокое понимание curl](https://starkovden.github.io/understand-curl.html)

## Командная строка

- [Полезные команды в cmd на Windows](https://htmlacademy.ru/blog/soft/windows-command-line)
- [Терминал для начинающих: полезные команды для работы в командной строке Windows, Linux и MacOS](https://proglib.io/p/terminal-dlya-nachinayushchih-poleznye-komandy-dlya-raboty-v-komandnoy-stroke-windows-linux-i-macos-2023-03-20)

## Работа с Python

- [Запуск Python скрипта через командную строку](https://sky.pro/wiki/python/zapusk-python-skripta-cherez-komandnuyu-stroku/)
- [How to Use Python: Your First Steps](https://realpython.com/python-first-steps/#why-you-should-use-python)
- [Питон, панды и анаконда: что внутри у Jupyter Notebook](https://practicum.yandex.ru/blog/chto-takoe-jupyter-notebook/)

## Консоль разработчика в браузере

- [Гайд по DevTools: как открыть инструменты разработчика в браузере и чем они полезны](https://practicum.yandex.ru/blog/devtools-instrumenty-razrabotchika/#poleznye-funkcii)
- [Советы по инструментам разработчика](https://developer.chrome.com/docs/devtools/tips)

## Docker и Docker Hub

- [Почти все, что вы хотели бы знать про Docker](https://habr.com/ru/articles/822707/)
- [Docker. Работа с образами](https://gist.github.com/dnncomp/ad439e3128854c71fd6b8ac4bb68005c)
- [Как работает Docker: подробный гайд от техлида](https://skillbox.ru/media/code/kak-rabotaet-docker-podrobnyy-gayd-ot-tekhlida/)

## Работа с промптами

- [Как правильно писать промты? Основная структура](https://www.ai-lib.pro/tpost/iiidx7y6j1-kak-pravilno-pisat-promti-osnovnaya-stru)
- [Библиотека Anthropic](https://docs.anthropic.com/ru/prompt-library/)
- [Обучающий курс на любую тему](https://www.ai-lib.pro/tpost/os6n8nkcp1-obuchayuschii-kurs-na-lyubuyu-temu)
 
